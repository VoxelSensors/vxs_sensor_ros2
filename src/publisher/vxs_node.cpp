...
namespace vxs_ros
{
...

    {
        ...
        frame_polling_thread_ = embedded_triangulation_mode_ ? std::make_shared<std::thread>(std::bind(&VxsSensorPublisher::EmbeddedTriangPollingLoop, this)) : std::make_shared<std::thread>(std::bind(&VxsSensorPublisher::FramePollingLoop, this));
        RCLCPP_INFO_STREAM(this->get_logger(), "Done!");
    }
    ...
    void VxsSensorPublisher::EmbeddedTriangPollingLoop()
    {
        ...
        while (!flag_shutdown_request_)
        {
            ...
            uint16_t *buffer_16bits = reinterpret_cast<uint16_t *>(buffer[0]);

            PublishStampedPointcloudET(num_points, buffer_16bits);
        }
    ...
    void VxsSensorPublisher::PublishStampedPointcloudET(const int N, uint16_t *buffer_16bits)
    {
        // Setup the stamped pointcloud message
        sensor_msgs::msg::PointCloud2::SharedPtr msg = std::make_shared<sensor_msgs::msg::PointCloud2>();
        ...
        // Now, fill-in the pointcloud data
        uint8_t *ptr = &msg->data[0];
        for (int i = 0; i < N; i += 8)
        {
            if (*(buffer_16bits + i + 4) == 0)
            {
                continue;
            }
            vxsdk::vxXYZT e(*(buffer_16bits + i), *(buffer_16bits + i + 1), *(buffer_16bits + i + 4), *(uint32_t *)(buffer_16bits + i + 5));

            float *point = reinterpret_cast<float *>(ptr);
            point[0] = e.x;                                          // X coordinate
            point[1] = e.y;                                          // Y coordinate
            point[2] = e.z;                                          // Z coordinate
            *(double *)(ptr + t.offset) = *(double *)&(e.timestamp); // @TODO: What on earth is this 64-bit time?
            // Advance in the message data buffer
            ptr += msg->point_step;
        }
